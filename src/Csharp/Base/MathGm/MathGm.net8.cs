...
public static partial class MathGm
{
    ...
    public static (T, T, T) Slerp<T>(T v1X, T v1Y, T v1Z, T v2X, T v2Y, T v2Z, T t) where T : IFloatingPointIeee754<T>
    {
        T dot = v1X * v2X + v1Y * v2Y + v1Z * v2Z;
        dot = T.Clamp(dot, -T.One, T.One);
        if (dot > T.One - Cache<T>.T1e5)
        {
            T x = v1X + t * (v2X - v1X);
            T y = v1Y + t * (v2Y - v1Y);
            T z = v1Z + t * (v2Z - v1Z);
        ...
        T factor1 = T.Sin((T.One - t) * omega) / SinOmega;
        T factor2 = T.Sin(t * omega) / SinOmega;
        return (
